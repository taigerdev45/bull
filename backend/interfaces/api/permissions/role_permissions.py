...
class IsAdmin(permissions.BasePermission):
    def has_permission(self, request, view):
        if not (request.user and request.user.is_authenticated):
            return False
        role = str(getattr(request.user, 'role', '')).lower().strip()
        return role in ['super_admin', 'admin'] or request.user.is_superuser or request.user.email == 'taigermboumba@gmail.com'
...
class IsSecretariat(permissions.BasePermission):
    def has_permission(self, request, view):
        if not (request.user and request.user.is_authenticated):
            return False
        role = str(getattr(request.user, 'role', '')).lower().strip()
        return role in ['super_admin', 'admin', 'secretariat'] or request.user.is_superuser or request.user.email == 'taigermboumba@gmail.com'
